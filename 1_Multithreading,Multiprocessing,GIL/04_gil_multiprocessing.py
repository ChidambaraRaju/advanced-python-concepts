'''
GIL:
    - In threading, all threads share the same GIL, so only one thread can execute Python code at a time
    - In multiprocessing, each process has its own Python interpreter and therefore its own GIL.
